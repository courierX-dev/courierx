...
import { apiKeysService, type ApiKey, type CreatedApiKey } from "@/services/api-keys.service"
...
export default function ApiKeysPage() {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    setCreating(true)
    try {
      const newKey = await apiKeysService.create({ name: keyName })
      setCreated(newKey)
  ...
  async function handleRevoke() {
    ...
    setRevoking(true)
    try {
      const updated = await apiKeysService.revoke(revokeTarget.id)
      setKeys((prev) => prev.map((k) => k.id === revokeTarget.id ? updated : k))
      ...
      <Dialog open={!!revokeTarget} onOpenChange={(v) => { if (!v) setRevokeTarget(null) }}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 mt-2">
            <div className="rounded-md border border-destructive/20 bg-destructive/[0.04] px-4 py-3 space-y-1">
